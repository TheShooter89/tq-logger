...
const OPTIONS = {
  ...
  const logger = (component_name, global_options) => {
    ...
    /*
    log_wrapper arguments structure:
    ...
    */
    ...
    const log_wrapper = (...[message, log_level, log_section, ...rest]) => {
      ...
      let level = check_level(log_level)
        ? check_level(log_level)
        : additional_params.push(log_level) && "INFO";
      let section = check_section(log_section)
        ? check_section(log_section)
        : additional_params.push(log_section) && null;
